...
async function simulateDistribution(payload = {}) {
  const config = publicConfig();
  const options = distributionOptions(payload);
  const storedSnapshot = options.snapshotId ? await readRecord("snapshots", options.snapshotId) : await latestRecord("snapshots");
...
async function finalizeManifest(payload = {}) {
  const options = distributionOptions(payload);
  let snapshot = options.snapshotId ? await readRecord("snapshots", options.snapshotId) : await latestRecord("snapshots");
  if (!snapshot) {
  ...
  const existingManifest = await latestRecord("manifests");
...
async function generateDistributionBatch(payload = {}) {
  const body = payload.payload || {};
  const manifest = await readRecord("manifests", body.manifestId || body.snapshotId || "latest");
  if (!manifest) {
  ...
  const existing = await latestRecord("batches");
...
async function distributeWbtc(payload = {}) {
  const body = payload.payload || {};
  const batch = await readRecord("batches", body.batchId || "latest");
  if (!batch) {
