...
function publicConfig(env = process.env) {
  const wallet = env.WALLET || env.PUBLIC_FEE_WALLET || env.DEV_CREATOR_WALLET || DEFAULT_PROJECT_WALLET;
  const distributorWallet = env.PUBLIC_DISTRIBUTOR_WALLET || env.DISTRIBUTOR_WALLET || wallet;
  const tokenMint = env.TOKEN_MINT || env.PUBLIC_TOKEN_MINT || DEFAULT_PROJECT_TOKEN_MINT;
  const wbtcMint = env.REWARD_MINT || env.PUBLIC_WBTC_MINT || DEFAULT_WBTC_MINT;
  ...
  return {
    ...
    jupiterSwapUserPublicKey: env.JUPITER_SWAP_USER_PUBLIC_KEY || distributorWallet || wallet,
    creatorFeeClaimPublicKey: env.CREATOR_PUBLIC_KEY || wallet,
...
async function feeReceipts(env = process.env) {
  ...
  if (!solanaRpcUrl(env)) {
    return launchSafeStatus("awaiting_rpc", "Fee wallet is set; live receipt polling starts when SOLANA_RPC_URL is configured.", {
...
async function tokenBalance(owner, mint, env = process.env) {
  ...
  if (!solanaRpcUrl(env)) {
    return launchSafeStatus("awaiting_rpc", "Token balance polling starts when SOLANA_RPC_URL is configured.", {
      owner,
      mint,
      accountCount: 0,
      balance: null
