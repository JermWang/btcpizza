...
function renderReceipts() {
  ...
  const receipts = [];
  // pull from rewardReceipts, feeReceipts, operations.receipts when available
  state.rewardReceipts.forEach((r) => receipts.push({ status: r.status, time: r.created_at, signature: r.signature, label: r.amount_ui ? `${r.amount_ui} NVDAx` : "" }));
  state.feeReceipts.forEach((r) => receipts.push({ status: r.status, time: r.blockTime ? new Date(r.blockTime * 1000).toLocaleString() : "", signature: r.signature, label: "Fee receipt" }));
  ...
  receipts.slice(0, 8).forEach((r) => {
    ...
    const link = r.signature
      ? `<a class="tx-link" href="https://solscan.io/tx/${r.signature}" target="_blank" rel="noreferrer">TX ${shortAddress(r.signature)}</a>`
...
async function loadRewardsStatus() {
  try {
    ...
    if (els.pool) els.pool.textContent = s.total_nvdax_pool && s.total_nvdax_pool !== "0" ? s.total_nvdax_pool : "0.00";
    if (els.creatorFees) els.creatorFees.textContent = s.creator_fees_collected || "$0";
  } catch {}
}
async function loadHolderSnapshot() {
  try {
    const r = await fetch("/api/rewards/holders", { cache: "no-store" });
    if (!r.ok) return;
...
// Tell tweaks panel which knobs to render once it boots
window.__jensenReady = true;
